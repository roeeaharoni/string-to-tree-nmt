...
def plot_head_map(mma, target_labels, source_labels, mma2=None, target_labels2=None, source_labels2=None):
    if mma2 == None:
        fig, ax = plt.subplots()
    else:
...
def reordering_score(attn_matrix):


# TODO: a method to measure amount of reordering:
# TODO: sum weight * (distance from diagonal), normalize by matrix size
# TODO: distance from diagonal = distance in y for same x on the diagonal line

def read_plot_alignment_matrices(f, comp=None):

    while (f):
        sid, mma, target_labels, source_labels = read_alignment_matrix(f)
        if comp:
            sid2, mma2, target_labels2, source_labels2 = read_alignment_matrix(comp)

        row_counter = 0
        # remove rows of tree symbols
        rows = []
        for i, t in enumerate(target_labels):
            if u'(' not in t and u')' not in t:
                try:
                    rows.append(mma[i, :])
        ...
        if comp:
            ...
            plot_head_map(no_tree_matrix, no_tree_target_labels, source_labels, mma2, target_labels2, source_labels2)
        else:
            plot_head_map(no_tree_matrix, no_tree_target_labels, source_labels)
...
if __name__ == "__main__":
    ...
    if args.json:
        read_plot_alignment_json(args.input, 9999)
    else:
        read_plot_alignment_matrices(args.input, args.comp)
